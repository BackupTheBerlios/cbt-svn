...
class DomainRecord(WhoisRecord):
    ...
    def ParseWhois_Generic(self):
        refer_cre = re.compile(r'ReferralServer: whois://(\S+)',re.I)
        m = refer_cre.search(self.page)
        while m:
            self.whois(self.domain,m.group(1))
            m = refer_cre.search(self.page)
