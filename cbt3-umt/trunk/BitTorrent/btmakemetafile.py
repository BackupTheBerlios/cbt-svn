...
from os.path import getsize, split, join, abspath, isdir, dirname
...
def make_meta_file(file, url, piece_len_exp, 
		flag = Event(), progress = dummy, progress_percent=1, comment = None, target = None, f = None):
	...
	data = {'info': info, 'announce': strip(url), 'creation date': long(time()), 'cbt_path':upath, 'cbt_user': cbt.encode('latin-1')}
	if comment:
