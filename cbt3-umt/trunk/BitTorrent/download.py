...
class Download:
    ...
    def setup(self, params, filefunc, statusfunc, finfunc, errorfunc, doneflag, 
        cols, pathFunc = None, paramfunc = None, spewflag = Event(), callback = None, btconfig=None):
        ...
        if len(params) == 0:
            ...
            return False
        try:
            config, args = parseargs(params, defaults, 0, 50)
            if args:
        ...
        try:
            ...
            if info.has_key('length'):
                ...
                try:
                	rsp = response['umt_user']
                except:
                	rsp = ''
                
                if rsp == self.btconfig['umt_login']:
                    sv = response['umt_path']
                else:
                    sv = file

                config['saveas'] = sv #g3rmz
                if file is None:
                    return False
                make(file)
                ...
                if path.exists(file):
                    for x in info['files']:
                        try:
                            ...
                            #handle Unicode Decode or Encode error here
                            pass
                                    
                    if not existing:
                        file = path.join(file, info['name'].decode('latin-1'))
                ...
                for x in info['files']:
                    n = file
                    for i in x['path']:
                        n = path.join(n, i.decode('latin-1'))
    ...
    def download(self, params, filefunc, statusfunc, finfunc, errorfunc, \
            friendfunc, doneflag, cols, my_id=None, pathFunc=None, \
            paramfunc=None, spewflag=Event(), onstartfunc=None):
        ...
        DownloaderFeedback(choker, rawserver.add_task, statusfunc, 
            ...
            config['display_interval'], spewflag, picker, storagewrapper)
...
#            if response.get('umt_user') == self.btconfig['umt_login']:
#                sv = response.get('umt_path')
#            else:
#                sv = config.get('saveas')
            
            pass2start = {
                'peer_id' : myid,
                'info_hash' : infohash,
                'listen_port' : listen_port,
                'announce' : response.get('announce'),
                ...
                'umt_path': response.get('umt_path'),
                'umt_user': response.get('umt_user'),
...
#------------------------------------------------------------------------------
    ...
    def SetPieceRanges(self, ranges):
        ...
        self.rawserver.external_add_task(foo, 0)
    ...
    def SelectChoker(self, selection, choke_period, opt_unchoke_period, min_rate):
        if self.doneflag.isSet():
            return
        self.choker.SelectChokeAlgo(selection, [choke_period, opt_unchoke_period, min_rate*1024])
